...
export type ThemedInputProps = TextInputProps & {
  ...
  validation?: (value: string) => string | null; // Validation function
  error?: string | null; // External error from parent
  minValidationLength?: number; // Optional minimum length before validating
};
...
export function ThemedTextInput({
  ...
  validation,
  error: externalError,
  minValidationLength = 1, // Default: validate from first character
  onChangeText,
  ...otherProps
}: ThemedInputProps) {
  const backgroundColor = useThemeColor(
  ...
  const handleChangeText = (text: string) => {
    ...
    // Live validation only after a certain length
    if (validation) {
      if (text.length >= minValidationLength) {
        setInternalError(validation(text));
      } else {
        setInternalError(null); // Clear error if not enough characters
      }
    }
  ...
  return (
    <View style={{ minHeight: 48, flex: 1, marginBottom: 26 }}>
      ...
      />
      {finalError && (
        <Text style={{ color: "red", fontSize: 12, marginTop: 2 }}>{finalError}</Text>
      )}
    </View>
  );
}
